...
3. The signer tries to retrieve and verify the consent message according to the [ICRC-21](ICRC-21/icrc_21_consent_msg.md) specification. If the canister call is not an ICRC standard supported by the signer and the target canister does not support ICRC-21 consent messages for the call that can be used as fallback, the signer may proceed in one of following ways:
