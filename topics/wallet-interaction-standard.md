...
`challenge` (`blob`): A challenge used for the wallet to sign in order to prove its access to the identity. The challenge should be an array of 32 cryptographically random bytes generated by the sender of the request.
...
- `algorithm` (`text`): The cryptographic algorithm used to generate the key pair associated with the identity. The supported values correspond to signature schemes [supported by the IC](https://internetcomputer.org/docs/current/references/ic-interface-spec/#signatures) and include:
    - `"ed25519"`
    - `"secp256r1"`
    - `"secp256k1"`
...
`challenge` (`blob`): The challenge the wallet had to sign.

`signature` (`blob`): The signature produced by signing the challenge with the private key associated with the `identity`.
...
3. After receiving a succesful response, the relying party verifies that the wallet has access to the private key associated with the provided identity:
    - The relying party checks if the `challenge` from the response matches the `challenge` sent in the request.
        - If the `challenge` values do not match, the relying party rejects the connection.
    - The relying party retrieves the `publicKey` and `algorithm` from `identity`.
    - Using the `algorithm`, the relying party determines the `signature` scheme and verifies whether it was generated by signing the `challenge` with the private key associated with the `publicKey`.
...
// Request
{
    ...
    "params": {
        ...
        "challenge": "3Q/X4Vim3kAhAiLzdHi8h/AC1nzpDyfT4D/6LpxFIkc="
    }
}
...
// Response
{
    ...
    "result": {
        ...
        "identity": {
            "algorithm": "secp256k1",
            "publicKey": "MFYwEAYHKoZIzj0CAQYFK4EEAAoDQgAEXk2ugrQNYq22iizREPgnphpMG+dpFw4JRCAlQ++uP8+/fvYdXyPkDkNHT2p6Wo3lipX4KR5D4K9PmT2XEIRxbg==" /* principal: 7aihj-fuxxk-cumes-wt4rz-n4ijk-y5x43-bvrm3-4axgi-ikq6z-57xv3-hae */
        },
        "challenge": "3Q/X4Vim3kAhAiLzdHi8h/AC1nzpDyfT4D/6LpxFIkc=",
        "signature": "rEm2+pM0LdlWP3Rvyry1uD9YcLUGI4eaxENqYPcTaP8Wj12v7SEIWX/fokcy99XnPQIo879J0+7HP1I1UgDB3w=="
    }
}
```
...
2. Upon receiving the request, the wallet validates whether the relying party has the permission to request the action and processes the message following the [ICRC-21]() specification.
    - If the user approves the request:
        - The wallet sends the call to the IC, retrieves its content map ([call](https://internetcomputer.org/docs/current/references/ic-interface-spec/#http-call), [query](https://internetcomputer.org/docs/current/references/ic-interface-spec/#http-query)) and [calculates a request id](https://internetcomputer.org/docs/current/references/ic-interface-spec/#request-id) based on it.
...
3. If the response is successful, the relying party verifies whether the call performed by the wallet was genuine and retrieves the result:
    ...
    - The relying party uses the calulcated request id to retrieve [the `reply` blob](https://internetcomputer.org/docs/current/references/ic-interface-spec/#state-tree-request-status) from the `certificate`'s state tree.
...
// Request
{
    ...
    "params": {
        ...
        "sender": "7aihj-fuxxk-cumes-wt4rz-n4ijk-y5x43-bvrm3-4axgi-ikq6z-57xv3-hae",
        "method": "transfer",
...
// Response
{
    ...
    "result": {
        ...
        "contentMap": {
            "request_type": "call",
            "sender": "l7qFRhJWnyOW8QlWO35sNYs3wFzIQqHs9/euzgI=",
            "nonce": "AAABifRDOG6dc4ueGJPwKg==",
            "ingress_expiry": "1692020433390000000",
            "canister_id": "gAAAAAAQAAEBAQ==",
            "method_name": "transfer",
            "arg": "RElETARte24AbAKzsNrDA2ithsqDBQFsA/vKAQKi3pTrBgHYo4yoDX0BAwEdrH2v4C9riZI1Ss2DBLYdFDnt53DN2OUDJIiEgQIAAOgH"
        },
        "certificate": ""
    }
}
```
